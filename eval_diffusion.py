...
def parse_args_and_config():
    ...
    parser.add_argument('--merge', default="False", type=str,
                        help='Merge the patch')
...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=config.sampling.batch_size,
                                              shuffle=False, num_workers=6)
    print("=> creating denoising-diffusion model with wrapper...")
    diffusion = DenoisingDiffusion(args, config)
    model = DiffusiveVAD(diffusion, args, config)
    model.restore_video(test_loader, r=args.grid_r)
